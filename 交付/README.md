...
**六只版的静帧与视频用的不是同一个机位**，所以 `render_delivery.py` 里那一步分两趟跑。
视频跟拍第一只（整队同速、队形不散，跟着走才不会跑出画面）；
但跟拍把被跟的那只摆在画面正中，而它在群体一端 —— 静帧于是只装得下四五只，
另一端那只被边缘切掉。实测拉距离治不了偏心（间距 0.8/距离 3.4、间距 0.5/距离 2.2 与 2.6
三档全部照切，只是鸭子越来越小）。静帧改走 `parallel_camera()` 量出来的世界机位：
先跑十二步量出这六只的实际位置，按群体中心与主轴跨度定距离和方位，六只居中、一只不缺。
